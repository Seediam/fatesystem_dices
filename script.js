(function () {
  ...
  function waitForOBRReady(timeoutMs) {
    return new Promise((resolve) => {
      ...
      OBR.onReady(() => {
        const current = getOBR();
        finish(Boolean(current?.player && typeof current.player.setMetadata === "function"));
      });
  ...
  async function sendRumbleChatViaMetadata(texto, senderName) {
    const OBR = getOBR();
    if (!OBR?.player || typeof OBR.player.setMetadata !== "function") return;
  ...
  if (botaoRolar) {
    botaoRolar.addEventListener("click", async (event) => {
      ...
      try {
        ...
        if (!ready) {
          setStatus("OBR indisponível. Reabra a extensão e tente novamente.", "#f0b90b");
          return;
        }
